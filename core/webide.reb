...
websocket-obj: make object! [
    ...
    compute: func [ key [ string! binary! ]] [
...
;probe reduce [length2 length1 xor-length]
                ...
                repeat i xor-length [
                    either any [
                        none? payload-data/:i
                        ;none? masking-key/((int-mod (i - 1) 4) + 1)
                    ][
                        error-length?: true
                        break
                    ][
...
tcp-server-client-obj: make object! [
    ...
    start: func [tcp-target data-handler client-closed-handler][
        ...
        forever [
            either server? [
                ...
                active-port: first wait accept-port
                either active-port/remote-ip = 127.0.0.1 [
                    close accept-port ;-- only accept one client
                    output-log join "connection from: " [active-port/remote-ip ":" active-port/remote-port " at " now/time/precise]
                ][
                    active-port: none
                ]
            ][
                active-port: open/no-wait/binary/direct to-url tcp-target
            ]
            forever [
                data: copy wait active-port
                either send-ready?: not any [none? data empty? data] [
                    data-handler active-port data
                ][
                    output-log "client closed"
                    break
                ]
            ]
    ...
    start2: func [tcp-target data-handler client-closed-handler][
        accept-port: none
        either none? tcp-target: tcp-target [
            tcp-target: ask "input tcp target (like tcp://:8080 or tcp://127.0.0.1:8080) ? "
        ][
        ...
        forever [
            either server? [
                ...
                if not none? wait [tcp-wait accept-port] [
                    ...
                    if active-ports/1/remote-ip <> 127.0.0.1 [
                        head remove active-ports
                    ]
                ]
            ][
...
webide: make object! [
    ...
    if os-id = 3 [
        ...
        utf16be-to-utf8: func [bin16 [binary!] /local bin8 code-point count offset temp][
            ; func   by http://www.w3.org/TR/encoding/#utf-8-encoder
            ; check1 by http://www.haomeili.net/Code/DetailCodes?wd=%E4%B8%AD%E6%96%87%2F
            ; check2 by http://macchiato.com/unicode/convert.html
            bin8: make binary! 0
    ...
    executor: make object! [
        ...
        replace-path-spaces: func [str][
            replace/all replace/all replace/all copy str " " "%20" "(" "%28" ")" "%29"
        ]
        kill-then-call: func [str-kill-first str-cmd /need-wait /no-run /keep-console /local all-str-cmd cmd-kill cmd-before cmd-after][
            all-str-cmd: make string! 0
            unless none? str-kill-first [
                case [
                    3 = os-id [
                        either keep-console [
                            cmd-kill: rejoin ["taskkill /f /t /im " "cmd"          ".exe"]
                        ][
                            cmd-kill: rejoin ["taskkill /f /t /im " str-kill-first ".exe"]
                        ]
                    ]
            ...
            case [
                ...
                2 = os-id [
                    ...
                    cmd-after:  {"'}
                    head insert str-cmd {\}
                    head insert skip tail str-cmd -1 {\}
                    replace/all str-cmd {" -i -v -s "} {\" -i -v -s \"} ;for "compile"
                    replace/all str-cmd {" "} {\" \"}                   ;for "do"
                ]
                4 = os-id [
            ...
            unless no-run [
                either all [3 = os-id    keep-console] [
                    temp-bat: rejoin [script-dir os-dir %_temp_interpret.bat]
                    write/binary temp-bat rejoin ["@ECHO OFF^(0D)^(0A)" all-str-cmd] ; "^(0D)^(0A)pause"]
                    call/show probe rejoin [{cmd.exe /k "} to-local-file temp-bat {"}]
                ][
        ...
        compile: func [output-file target-os debug-flag file /call-new-rebol /then-run /need-wait /local all-str-cmd str-do temp-reb temp-reb-str][
            ...
            str-do: rejoin [{do/args %} replace-path-spaces red-r { "-o %} replace-path-spaces output-file { -t } target-os { } debug-flag { %} replace-path-spaces file {"}]
            if then-run [
                case [
                    3 = os-id [append str-do rejoin ["^(0D)^(0A)call/show      {cmd.exe /k ^"" to-local-file output-file "^"}"]]
                    4 = os-id [append str-do rejoin ["^(0D)^(0A)call/show/wait {           ^"" to-local-file output-file "^"}"]]
                    true      [append str-do rejoin ["^(0D)^(0A)call/show      {           ^"" to-local-file output-file "^"}"]]
                ]
            ]
            either call-new-rebol [
                temp-reb: rejoin [script-dir os-dir %_temp_compile.reb_]
                temp-reb-str: rejoin ["REBOL [] ^(0D)^(0A)" str-do "^(0D)^(0A)"]
                write/binary temp-reb probe temp-reb-str
                either need-wait [
                    kill-then-call/need-wait none rejoin [{"} to-local-file rebol-exe {" -i -v -s "\"} temp-reb {\""}]
                ][
                    kill-then-call           none rejoin [{"} to-local-file rebol-exe {" -i -v -s "\"} temp-reb {\""}]
                ]
            ][
                do probe str-do
            ]
        ]
        create-temp-file: func [editing-file /local temp-file bin blk][
            temp-file: rejoin [script-dir %_temp.red_]
            bin: make binary! 0
            code-before: [
Red []
http-post: func [value][
    ; Red 0.6.0 http only support windows platform
    if 'windows = system/platform [
        result: write http://127.0.0.1:8080/Red_WebIDE.html reduce [
            'post
            [
                Connection:   "close"
                Referer:      http://127.0.0.1:8080/Red_WebIDE.html
                Content-Type: "application/x-www-form-urlencoded"
            ]
            value ;can not use binary, Red bug, but Rebol23 ok
        ]
    ]
]
sys-prin: :prin
prin2: func [value][
    sys-prin value
    http-post append make string! 0 form reduce value
]
sys-print: :print
print2: func [value][
    sys-print value
    http-post append make string! 0 form reduce [reduce value newline]
]
sys-probe: :probe
probe2: func [value][
    sys-probe value
...
replace-print-probe: func [][
    prin:  :prin2
    print: :print2
    probe: :probe2
]
replace-print-probe
try-block: func [blk bind-args /local str try-block arg][
    str: ""
    try-blk: compose/deep [
        if error? set/any 'result try [
            str: (blk)
        ][
            either value? 'disarm [
                str: mold disarm get/any 'result
            ][
                str: mold get/any 'result
            ]
        ]
    ]
    if not empty? bind-args [
        foreach arg bind-args [
            bind try-blk arg
        ]
    ]
    do try-blk
    str
            ...
            append bin to binary! mold/only code-before
            append bin to binary! "^(0D)^(0A)blk: [^(0D)^(0A)^(0D)^(0A)^(0D)^(0A)"
            append bin read/binary rejoin [work-dir editing-file]
            append bin to binary! "^(0D)^(0A)^(0D)^(0A)^(0D)^(0A)]"
            code-after: [
re: try-block blk none
print re
;print append make string! 0 reduce ["== " re]
            ]
            append bin to binary! mold/only code-after
            write/binary temp-file bin
            temp-file
            ; do %/C/Users/a/Desktop/Red_WebIDE/core/_temp.red_
        ]
        blk-unzip: make block! 0
        data-handler: func [keys value /local reply][
            reply: none
            case [
                ...
                "do" = keys/1 [
                    ...
                    either find keys/2 "kill" [
                        kill-then-call/keep-console "red-console" str-call
                    ][
                        kill-then-call/keep-console none          str-call
                    ]
                    reply: make string! 0
                ]
                "compile" = keys/1 [
                ...
                ;"list-unzip" = keys/1 [
                ;    blk-unzip: parse/all value "|"
                ;    reply: to string! length? value
                ;]
                ;"write-unzip" = keys/1 [
                ;    unzip-offset: 0
                ;    forever unzip blk-unzip [
                ;        unzip-info: parse/all unzip ":"
                ;        unziped-file: rejoin [script-dir "red-source/" unzip-info/1]
                ;        make-dir/deep file-viewer/go-parent unziped-file
                ;        write/binary probe unziped-file copy/part skip value unzip-offset unzip-info/2
                ;        unzip-offset: unzip-offset + unzip-info/2
                ;    ]
                ;    reply: to string! length? value
                ;]
    ...
    data-handler: func [active-port data][
        case [ ;case1 must be front than case2
            ...
            any [
                all [buffer: "GET /"  equal? buffer to string! copy/part data length? buffer]
                all [buffer: "POST /" equal? buffer to string! copy/part data length? buffer]
            ][
                active-port-http: active-port
                bin: none
                case [
                    "GET /" = buffer [
                        file-name: none
                        parse data [thru "/" copy file-name to " "]
                        parse file-name [thru "." [
                                "html" (mime: "text/html") |
                                "css"  (mime: "text/css")  |
                                "js"   (mime: "application/javascript") |
                                "zip"  (mime: "application/octet-stream")
                            ]
                        ]
                        if url-query: find file-name "?" [
                            file-name: copy/part file-name index? back url-query
                        ]
                        probe html-dir/:file-name
                        either exists? html-dir/:file-name [
                            bin: read/binary html-dir/:file-name
                            insert bin rejoin ["HTTP/1.0 200 OK^(0A)Content-type: " mime "^(0A)^(0A)"]
                        ][
                            print "not exists"
                        ]
                    ]
                    "POST /" = buffer [
                        bin-length: 0
                        parse data [
                            thru "Content-Length: "     copy bin-length to "^(0D)^(0A)^(0D)^(0A)"
                            thru "^(0D)^(0A)^(0D)^(0A)" copy bin        to end
                        ]
                        if ws-send-ready? [
                            ws-send active-port-ws #{82} dto-out "server-push" bin
                        ]
                    ]
                ]
                tcp-server-client-obj/send active-port-http bin
                tcp-server-client-obj/remove-active-port active-port-http
            ]
            true [
                ws-send-ready?: true
...
;write/binary %temp.txt bin
                either websocket-obj/error-length? [
                    probe "error-frame-length! auto retry..."
...
;probe dto
                    ...
                    either websocket-obj/data-frame? [
                        ...
                        either none? reply [
                            ;unknown key of breaken frame
                        ][
                            either find dto/1 "write-one-unzip|" [
...
webide/file-viewer/browse to-local-file join webide/file-viewer/go-parent system/script/path %ui/WebIDE-Red.html   ;-- for debug   (many files, firefox no work in "upgrade" button)
;webide/file-viewer/browse http://127.0.0.1:8080/WebIDE-Red.html                                                    ;-- for release (one html)
...
;-- REBOL [
;--     title: "Simple TCP Port-Forwarder"
;--     author: "http://www.rebol.net/cookbook/recipes/0055.html"
;-- ]
;-- ; do %/C/Users/a/Desktop/WebIDE-Red-Windows/core/example/tcp.reb
;-- port-id: 9999
;-- server: open/no-wait/binary/direct to url! rejoin ["tcp://" ":" port-id]
;-- ;web browser is smart at https, need to add exception to web browser
;-- ;browse rejoin ["https://127.0.0.1:" port-id "/red/red/zipball/master/"]
;-- remote: open/no-wait/binary/direct to url! rejoin ["tcp://" read dns://github.com ":443"]
;-- if server-port: wait server [
;--     probe client: first server-port
;--     close server-port
;-- ]
;-- until [
